...
use crate::models::{
  AckResponse, AppState, GetSessionsQuery, MemoryMessage, MemoryMessagesAndContext, MemoryResponse,
  NamespaceQuery,
};
    ...
    if !session_cleanup.get(&*session_id).unwrap_or(&false) {
      ...
      tokio::spawn(async move {
        ...
        let _compaction_result =
          handle_compaction(session_id.to_string(), model, window_size, client, conn).await;
